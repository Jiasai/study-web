...
class Login extends Component {
    ...
    handleModalShow() {//显示modal
        this.setState({ modalshow: true })
    }
    handleModalhide() {//隐藏modal
    ...
    userinputWrite(e) { //输入用户名
        this.setState({ username: e.target.value });
    }
    passwordinputWrite(e) {//输入密码
        ...
        return (
            <Fragment>
                <div className='login'>
                    {
                        this.state?.login ? 
                        <Button type="primary" data-login={this.state.login} onClick={this.handleLogOut}>退出</Button> :
                        <Button type="primary" data-login={this.state.login} onClick={this.handleModalShow}>登录</Button>
                    }
                </div>
                <Modal title="登录"
