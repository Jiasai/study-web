...
class BaseSlider{
	...
	showOut(currIndex,Toindex,opacity){	
		...
		setTimeout(()=>{this.showIn(Toindex,1);},5);
	}
	...
	Move(Index){
		...
		if(Index <= this.maxIndex && Index>= this.minIndex) {
			...
			this.sliderContent.style.left = `${-Index * this.contentWid/2}px`;		
			//修改焦点
			this.FocusActive(Index);
			...
			this.sliderContent.style.transition = 'none'; 
			this.sliderContent.style.left = `${-(this.maxIndex+1) * this.contentWid/2}px`;
			setTimeout(()=>{
				//然后马上执行，往最后1张图的过渡动画；
				this.sliderContent.style.transition =`all ${this.PlaySpeed/1000}s ease 0s`;
				this.sliderContent.style.left = `${-this.maxIndex * this.contentWid/2}px`;	
	...
	switch(lockplay){		
		...
		this.sliderContent.addEventListener('mouseenter',()=>{
			//清除定时器自动播放
			clearInterval(this.timer);
		});
		this.sliderContent.addEventListener('mouseleave',()=>{
			//自动切换动画
			this.Autoplay(this.options.play);
		})
	}
	...
	FirstPic(){
		setTimeout(()=>{ 
			this.sliderItems[this.options.initIndex].className = `slider-item slider-item-active`;
		},300)
