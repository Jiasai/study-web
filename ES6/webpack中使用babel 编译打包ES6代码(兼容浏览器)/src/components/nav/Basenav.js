...
const DEFAULTS = {
	...
	cloneMenu: {
		...
		oneMenuLi: 'nav-item',           //参照克隆menu菜单的一级menu的li
		twoMenuUL: 'nav-two-ul',         //参照克隆menu菜单的二级menu的ul
		...
		if (this.showStyle === 'drop-down') {
			...
			for (const item of this.PC_oneMenuLi) {
				...
				if (item.lastElementChild.tagName.toLowerCase() === 'ul') {  // 如果li 的 子元素有 二级ul 
					...
					//组装html
					html += `<li class='${this.options.oneMenu.itemLi}'><a href='javascript:;' class='nav-mob-link'>${text}</a><ul class="${this.options.twoMenu.classname}"><li class='${this.options.cloneMenu.twoMenuLi}'><a href='${href}' class='nav-mob-link'>${text}</a></li>${twoliHtml}</ul></li>`;
