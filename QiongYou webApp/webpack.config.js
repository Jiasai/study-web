...
module.exports = {
	...
	//entry:'./src/pages/index/index.js',//入口文件，把src目录下的page/index/index.js文件 ，转化到 dist文件目录下的main.js文件(默认名)
 	...
 	entry: {								//指定多入口：
			  index:'./src/pages/index',		//把src目录下的index.js文件，转化到 dist文件目录下的index.js文件
			  destination:'./src/pages/destination',	
			  //把src下的destination.js，转化到 dist下的destination.js		
	...
	plugins:[									//使用插件，通过plugins属性设置 
		...
		new HtmlWebpackPlugin({					//每使用1个新插件，new 一下上面通过 require()引入的插件
			template:'./src/pages/destination/destination.art',//指定打包好的 .js文件，嵌入到src/pages/destination目录下的 list.art文件中，
			filename:'destination.html',				//释义：以list.art为模板复刻，复刻出来的文件叫list.html,			
			chunks:['destination']						//在复刻的list.html中，加载叫list的js文件（entry指定的）
		}),
